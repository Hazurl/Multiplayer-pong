/*
    All credits goes to Cristian Pallarés and Lukas Dürrenberger
    https://github.com/skyrpex/RichText
    Licensed under MIT and Public Domain
*/
...
{
    updateTextAndGeometry(m_texts.empty() ? nullptr : &m_texts.back(), text);
...
void RichText::Line::updateGeometry() const
{
    ...
    sf::Text* previous_text = nullptr;
    for (sf::Text& text : m_texts) {
        updateTextAndGeometry(previous_text, text);
        previous_text = &text;
    }
}
...
////////////////////////////////////////////////////////////////////////////////
void RichText::Line::updateTextAndGeometry(sf::Text* previous_text, sf::Text& text) const
{
    float offset = 0;
    if (previous_text) 
    {
        const sf::String& content = previous_text->getString();
        if (!content.isEmpty()) 
        {
            // Find last character's glyph
            sf::Uint32 character = content[content.getSize() - 1];
            const sf::Font& font = *text.getFont(); 
            const unsigned int character_size = previous_text->getCharacterSize();
            const sf::Glyph& glyph = font.getGlyph(character, character_size, previous_text->getStyle() & sf::Text::Bold);

            // Find kerning between the last character and the next one
            const sf::String& actual_content = text.getString();
            float kerning = actual_content.isEmpty() ? 0 : font.getKerning(character, actual_content[0], std::max(character_size, text.getCharacterSize()));

            offset = glyph.advance - glyph.bounds.width + kerning;
        }
    }

    // Set text offset
    text.setPosition(m_bounds.width + offset, 0.f);
    ...
    m_bounds.width += text.getGlobalBounds().width + offset;
}
